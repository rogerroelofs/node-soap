      ...
      for (var bindingName in bindings) {
        ...
        for (var methodName in methods) {
          if (methods[methodName].input) {
            ...
            if ( methods[methodName].input.children[0].name == 'part' ) {
              inputName = methods[methodName].input.children[0].$name;
              topEls[inputName] = {"methodName": methodName, "outputName": outputName};
            }
          }
        }
...
WSDL.prototype.xmlToObject = function(xml, callback) {
  ...
  p.onopentag = function(node) {
    ...
    if (!objectName && top.name === 'Body' && name !== 'Fault') {
      ...
      if (!message) {
        // look in parts
        const messagePart = Object.values(self.definitions.messages).filter(obj => {
          return ((obj.children) && (obj.children[0].$name == name));
        });
        if ( messagePart ) {
          const t = messagePart[0].$name;
          self.definitions.messages[originalName] = self.definitions.messages[t];
          message = self.definitions.messages[t];;
        }
      }
